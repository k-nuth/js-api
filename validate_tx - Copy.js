// https://chainquery.com/bitcoin-api/getrawtransaction

// 

const kth = require('./kth.js');
const express = require('express')
const app = express()

process.stdin.resume();

// const exec = kth.open("C:\\development\\btc-mainnet.cfg")
// const exec = kth.openWithStd("C:\\development\\btc-mainnet.cfg")
const exec = kth.openWithStd("")

var res = exec.initchain()
res = exec.run_wait()
// exec.stop()

// ---------------------------------

// if (res != 0) {
//     exec.close()
//     process.exit();
// }

process.on("SIGINT", function () {
    console.log("captured SIGINT...");
    process.exit();
});


process.on("exit", function () {
    // console.log("on exit");
    exec.close()
});

// ---------------------------------

app.get('/last-height', function(request, response) {
    var txhex = request.params.txhex;

    var height = exec.get_last_height()
    response.send(`last-height: ${height}`)
})

app.get('/validate-tx/:txhex', function(request, response) {
    var txhex = request.params.txhex;

    exec.validate_tx(txhex, function (err, message) {
        response.send(`err: ${err}, message: ${message}`)
    })
})

app.listen(8080, (err) => {
    if (err) {
        return console.log('something bad happened', err)
    }

    console.log(`server is listening on ${8080}`)
})

var prev_height = -1
var height = exec.get_last_height();
// while (height < 4000) {
// while (height < 8000) {
// while (height < 20000) {
// while (height < 40000) {
// while (height < 80000) {
// while (height < 98000) {
while (height < 99059) {
    if (height % 100 == 0 && prev_height != height) {
        console.log(`last-height: ${height}`)
    }

    prev_height = height
    height = exec.get_last_height();
}

console.log(`last-height: ${height}`)
exec.stop()
console.log(`stopped`)
exec.close()
console.log(`closed`)

// BX
// http://127.0.0.1:8080/validate-

// Satoshi TX
// http://127.0.0.1:8080/validate-




// test.js

// Windows: Se necesita Python 2.7 y MSVC
//
// npm install -g node-gyp
//
// node-gyp configure
// node-gyp build
// node test.js

