...
class GetHeaders {
    ...
    toNative() {
        const native = kth.chain_get_headers_construct(this.start, this.stop);
        return native;
    }
...
const fromNative = function(native, destroy = false) {
    const obj = new GetHeaders(
          kth.chain_get_headers_start(native)
        , kth.chain_get_headers_stop(native)
    );
    if (destroy) {
        destruct(native);    
    }
...
const fromData = function(version, data) {
    const native = kth.chain_get_headers_factory_from_data(version, data)
...
const toData = function(obj, version) {
    ...
    const res = kth.chain_get_headers_to_data(native, version)
    destruct(native);
    return res;
}
...
const hash = function(obj) {
    const native = obj.toNative();
    const res = kth.chain_get_headers_hash(native)
    destruct(native);
    return res;
}
...
const destruct = function(native) {
    kth.chain_get_headers_destruct(native);
}
